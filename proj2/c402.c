...
**/
	if((*RootPtr) == NULL){
		tBTNodePtr inserted_node = (tBTNodePtr) malloc(sizeof(struct tBTNode));
				if (!inserted_node){
					return;
				}
		(*RootPtr) = inserted_node;
		(*RootPtr)->RPtr = NULL;
		(*RootPtr)->LPtr = NULL;
	...
	while(act_node != NULL){
		if(act_node->Cont == Content){ //daná hodnota sa už v strome nachádza
			return;
		}
		else if(act_node->Cont < Content){ //posúvam sa v strome do prava
			if(act_node->RPtr == NULL){
				tBTNodePtr inserted_node = (tBTNodePtr) malloc(sizeof(struct tBTNode));
				if (!inserted_node){
					return;
				}
				inserted_node->RPtr = NULL;
				inserted_node->LPtr = NULL;
				inserted_node->Cont = Content;
				act_node->RPtr = inserted_node;
				return;
			}
			else{
...
**/
	...
	while(!SEmptyP(&StackPtr)){
		...
		if(left){
			...
			SPushP(&StackPtr, RootPtr);
			Leftmost_Postorder(RootPtr->RPtr, &StackPtr, &StackBool);
		}
		else{
			BTWorkOut(RootPtr);
		}
